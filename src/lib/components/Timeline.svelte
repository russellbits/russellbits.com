<script>
	...
	const tags = ['articles', 'websites']

	const beginning = 1997
	const years = []
	for (let i = 0; i < 40; i++) {
...
<div class="options-wrapper">
	...
	<div>
		<a class="header-link" href="https://github.com/the-web-history/css-timeline" target="_blank"
			>Github</a
		>
	</div>
</div>
		...
		<li>
			<div class={index % 2 == 0 ? 'direction-r' : 'direction-l'}>
				<div class="flag-wrapper">
					<span class="flag"
						><a title={repo.name} target="_blank" href={repo.url}>{repo.name}</a></span
					>
					<span class="time-wrapper"
						><span class="time"
							>{repo.created_at.getFullYear()}, {months[repo.created_at.getMonth()]}</span
						></span
					>
				</div>
				<div class="desc">
					...
					{#if repo.links && repo.links.length > 0}
						...
						{/each}
					{/if}
					{#if repo.hasOwnProperty('image')}
						<img src={repo.image} alt="Sample screenshot of web site." />
					{/if}
				</div>
			</div>
		</li>
